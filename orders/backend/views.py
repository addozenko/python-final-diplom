...
class ConfirmAccount(APIView):
    ...
    def post(self, request, *args, **kwargs):
        # проверяем обязательные аргументы
        print(type(request.data), list(request.data.keys()))
        if {'email', 'token'}.issubset(request.data):
            ...
            if token:
                ...
                return JsonResponse({'Status': False, 'Errors': 'Неправильно указан токен или email'}, json_dumps_params={'ensure_ascii': False})
...
class LoginAccount(APIView):
    ...
    def post(self, request, *args, **kwargs):
        if {'email', 'password'}.issubset(request.data):
            ...
            return JsonResponse({'Status': False, 'Errors': 'Couldnt authorize'})

        return JsonResponse({'Status': False, 'Errors': 'Указаны не все необходимые аргументы'}, json_dumps_params={'ensure_ascii': False})
...
class BasketView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        items_sting = str(request.data.get('items')).replace("'", '"')
        if items_sting:
            try:
                items_dict = load_json(items_sting)
            except ValueError:
                JsonResponse({'Status': False, 'Errors': 'Неверный формат запроса'}, json_dumps_params={'ensure_ascii': False})
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if items_sting:
            ...
            for order_item_id in items_sting:
